...
# to run: python fs_redrawer.py svgs/infinity.svg 150
...
class FourierEpicycles:
    ...
    def calculate_cn(self, n):
        ...
        """
        # TODO: implement this method
        exponential_part=np.exp(-1j * n * self.omega * self.t)
        integrand=self.signal*exponential_part
        c_n= np.trapezoid(integrand,self.t)/self.T
        return c_n
    ...
    def approximate(self, t):
        ...
        """
        # TODO: implement this method
        result=np.zeros(np.shape(t),dtype=complex)

        for n,c_n in self.coeffs.items():
            current_position= c_n* np.exp(1j*n*self.omega*t)
