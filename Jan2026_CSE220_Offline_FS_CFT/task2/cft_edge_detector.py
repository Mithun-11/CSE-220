...
class CFT2D:
    ...
    def compute_cft(self):
        ...
        ux=self.u[:,None]*self.x[None,:]
        vy=self.v[:,None]*self.y[None,:]

        cos_2pi_ux=np.cos(2*np.pi*ux)
        sin_2pi_ux=np.sin(2*np.pi*ux)
        cos_2pi_vy=np.cos(2*np.pi*vy)
        sin_2pi_vy=np.sin(2*np.pi*vy)

        Cx=np.trapezoid(
            self.I[None,:,:] * cos_2pi_ux[:,None,:] ,
            self.x,
            axis=-1
        )

        Sx=np.trapezoid(
            self.I[None,:,:] * sin_2pi_ux[:,None,:] ,
            self.x,
            axis=-1
        )

        real_F=np.trapezoid(
            Cx[None,:,:]*cos_2pi_vy[:,None,:]
            - Sx[None,:,:]*sin_2pi_vy[:,None,:],
            self.y,
            axis=-1
        )

        im_F=np.trapezoid(
            -Cx[None,:,:]*sin_2pi_vy[:,None,:]
            - Sx[None,:,:]*cos_2pi_vy[:,None,:],
            self.y,
            axis=-1
        )
    ...
    def plot_magnitude(self):
        ...
        """
        # TODO: implement this method
        real_F,im_F=self.compute_cft()
        magnitude=np.sqrt(real_F**2+ im_F**2)
...
class InverseCFT2D:
    ...
    def reconstruct(self):
        ...
        """
        # TODO: implement this method
        ux=self.x[:,None]*self.u[None,:]
        vy=self.y[:,None]* self.v[None,:]
        ...
        Re_vx=np.trapezoid(
            self.real[:,None,:]*cos_2pi_ux[None,:,:]
            -self.imag[:,None,:]*sin_2pi_ux[None,:,:] ,
            self.u,
            axis=-1
        )

        Im_vx=np.trapezoid(
            self.real[:,None,:]*sin_2pi_ux[None,:,:]
            +self.imag[:,None,:]*cos_2pi_ux[None,:,:] ,
            self.u,
            axis=-1
        )

        signal=np.trapezoid(
            Re_vx[None,:,:]*cos_2pi_vy[:,:,None] 
            -Im_vx[None,:,:]*sin_2pi_vy[:,:,None],
            self.v,
            axis=1
        )
