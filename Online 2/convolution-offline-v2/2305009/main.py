...
def make_signal(start_time, end_time, values):
    ...
    for i,t in enumerate(signal.times()):
...
def signal_from_samples(start_time, end_time, samples):
    ...
    for t,val in samples.items():
        signal.set_value_at_time(t,val)
...
def impulse_identity():
    impulse_signal = DiscreteSignal(0,0)
    impulse_signal.set_value_at_time(0,1)
...
def impulse_moving_average(length):
    impulse_signal=DiscreteSignal(0,length-1)

    for i in range(length):
        impulse_signal.set_value_at_time(i,1/length)
...
def impulse_weighted_smoothing():
    impulse_signal = DiscreteSignal(0,2)
    impulse_signal.set_value_at_time(0,0.5)
    impulse_signal.set_value_at_time(1,0.3)
    impulse_signal.set_value_at_time(2,0.2)
    return impulse_signal
...
def impulse_first_difference():
    impulse_signal = DiscreteSignal(0,1)
    impulse_signal.set_value_at_time(0,1)
    impulse_signal.set_value_at_time(1,-1)
    return impulse_signal
...
def max_absolute_difference(first_signal, second_signal):
    ...
    for t in range (start, end+1):
        diff=max(abs(first_signal.get_value_at_time(t)-second_signal.get_value_at_time(t)),
                 diff)
